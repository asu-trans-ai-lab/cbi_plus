# -*- coding: utf-8 -*-
"""fd_model_zoo — 16 fundamental-diagram models for benchmarking against S3.

Integrated verbatim from the lab's public Traffic-Flow-Fundamental-Diagram
repo (Cheng, Liu, Lin, Zhou 2021 — the S3 paper's comparison suite):
S3, Greenshields, Greenberg, Underwood, Northwestern, GHR M1/M2/M3, KK,
Jayakrishnan, Van Aerde, MacNicholas, Wang 3PL/4PL/5PL, Ni — each with a
joint-estimation variant.

Role in cbi_plus: stage 3 fits S3 with Huber loss + bootstrap (production).
This zoo is the OPTIONAL benchmark layer — fit the alternatives on a sensor
where S3 looks wrong, and justify the model choice with numbers instead of
authority. Plain least-squares (scipy), as published.
"""
import numpy as np
